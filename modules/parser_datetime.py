...
def parser_datetime(text: str):
    ...
    # 時間をさがす(US)
    raw_times = re.findall(r"(?:2[0-3]|(?:0?|1)\d):[0-5]\d\-(?:2[0-3]|[0-1]?\d):[0-5]\d", fmt_txt)
    time_pairs = []
    for rt in raw_times:
    ...
    # 日付をさがす
    month_day = re.findall(r"(?:(?P<month>\d+)月)?(?P<day>\d+)日", fmt_txt)
    date_pairs = []
    for md in month_day:
        if md[0] == "":  # 月がない
